...
| subnets | List of subnet IDs to attach to the ALB | `list(string)` | `n/a` | yes |
...
| access_logs | Configuration for access logs: bucket, prefix, and enabled | `object({ bucket = string, prefix = optional(string), enabled = optional(bool) })` | `null` | no |
